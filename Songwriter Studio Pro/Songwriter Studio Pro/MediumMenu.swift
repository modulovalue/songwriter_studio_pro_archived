...
open class MediumMenu: UIView, UIGestureRecognizerDelegate {
    public enum State {
        case shown
        case closed
        case displaying
    ...
    open var simpleOffset: CGFloat = 150
    ...
    public init(items: [MediumMenuItem], forViewController: UIViewController) {
        ...
        if panGestureEnable {
            pan = UIPanGestureRecognizer(target: self, action: #selector(MediumMenu.panned(on:)))
    ...
    open override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if  gestureRecognizer === pan {
            let location = pan.location(in: contentController?.view)

            // CHANGED TO FIT NEEDS
            if location.y < UIScreen.main.bounds.height * 0.15 {
                return true
            } else {
                return false
            }
    ...
    fileprivate func open(animated: Bool = true, completion: (() -> Void)? = nil) {
        ...
        if animated {
            UIView.animate(withDuration: animationDuration, animations: {
                self.contentController?.view.center = CGPoint(x: x, y: self.screenHeight / 2 + self.height - self.simpleOffset)
            }, completion: { _ in
                UIView.animate(withDuration: self.animationDuration, animations: {
                    self.contentController?.view.center = CGPoint(x: x, y: self.screenHeight / 2 + self.height - self.bounceOffset - self.simpleOffset)
                }, completion: { _ in
